    ...
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      <div style={{ flex: 1, height: 4, background: theme.border, borderRadius: 999, overflow: "hidden" }}>
        <div style={{ width: `${score}%`, height: "100%",
          ...
          transition: "width 1s ease", boxShadow: `0 0 8px ${c}50` }} />
      </div>
      <span style={{ fontSize: 11, fontWeight: 800, color: c,
        fontFamily: "var(--font-geist-mono), monospace", minWidth: 36 }}>
        {score}/100
      </span>
    </div>
  );
...
export default function AgentReasoning({ results, steps, running, onSwap, lastRun }: AgentReasoningProps) {
  ...
  useEffect(() => {
    if (running) {
      ...
      setDisplayedSteps([
        "[SYSTEM] Connecting to Sentinel agent cluster...",
        "[SYSTEM] Launching market intelligence scan...",
      ]);
    }
  }, [running]);
  ...
  return (
    <div style={{ display: "grid", gap: 14 }}>
      ...
      <div className="terminal-chrome"
        style={{ background: "#07080F", border: `1px solid ${theme.border}`, borderRadius: 16, overflow: "hidden" }}>
        <div className="scan-line-overlay" />
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between",
          padding: "10px 16px", borderBottom: `1px solid ${theme.border}`,
          background: "rgba(13,15,26,0.5)", position: "relative", zIndex: 1 }}>
        ...
        <div style={{ padding: 16, fontFamily: "var(--font-geist-mono), monospace", fontSize: 12,
          lineHeight: 1.7, minHeight: 120, maxHeight: 280, overflowY: "auto", position: "relative", zIndex: 1 }}>
          <div style={{ color: theme.muted, marginBottom: 8 }}>
            <span style={{ color: theme.accent }}>sentinel</span>@
            <span style={{ color: theme.tierBasic }}>hashkey</span> ~/agent{" "}
          ...
          {displayedSteps.length === 0 && !running && results.length === 0 && (
            <div style={{ color: theme.muted }}>
              › Run an analysis cycle to see live market research and AI-generated trade signals from
          ...
          {displayedSteps.map((s, i) => {
            ...
            return (
              <div key={i} style={{
                color: isError ? theme.danger : isSuccess ? theme.accent : isInfo ? theme.textSecondary : theme.muted,
              }}>
                {s}
              </div>
            );
            ...
            {results.map((r, i) => {
              ...
              return (
                <div key={`${r.symbol}-${i}`} className="holo-card animate-fadeIn"
                  style={{ background: theme.panel, border: `1px solid ${theme.border}`, borderRadius: 16,
                  ...
                  <div style={{ position: "relative", zIndex: 1 }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between",
                      ...
                      <div>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                          {r.logoUrl && (
                            <img src={r.logoUrl} alt={r.symbol} width={18} height={18}
                              style={{ borderRadius: "50%" }}
                              onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
                          )}
                          <span style={{ fontSize: 18, fontWeight: 800, color: theme.text }}>{r.symbol}</span>
                          <span style={{ fontSize: 10, color: theme.muted, padding: "1px 6px",
                            background: theme.panelAlt, borderRadius: 4, border: `1px solid ${theme.border}` }}>
                            {r.chain === "hashkey" ? "🔑 HashKey" : r.chain === "base" ? "⚡ Base" : `Ξ ${r.chain?.toUpperCase()}`}
                          </span>
                          {res.isTrendingOnCoinGecko && (
                            <span className="neon-badge neon-badge-orange" style={{ fontSize: 9 }}>CoinGecko🔥</span>
                          )}
                          {res.isBoostedOnDexScreener && (
                            <span className="neon-badge neon-badge-blue" style={{ fontSize: 9 }}>DexScreener⚡</span>
                          )}
                          {r.chain === "hashkey" && (
                            <span className="neon-badge neon-badge-pink" style={{ fontSize: 9 }}>🔑 HSKSwap</span>
                          )}
                        </div>
                        <div style={{ fontSize: 11, color: theme.muted, marginTop: 2 }}>{r.name}</div>
                      </div>
                      <div style={{ textAlign: "right" }}>
                        <div style={{ display: "inline-flex", alignItems: "center", padding: "4px 12px",
                          borderRadius: 999, fontSize: 12, fontWeight: 700, color: c,
                          border: `1px solid ${c}40`, background: `${c}12`, boxShadow: `0 0 12px ${c}15` }}>
                          {r.action}
                        </div>
                        {res.risePct !== undefined && (
                          <div style={{ fontSize: 13, fontWeight: 800, color: theme.enter, marginTop: 4,
                    ...
                    <div style={{ display: "flex", gap: 14, marginTop: 10, fontSize: 12,
                      color: theme.muted, flexWrap: "wrap" }}>
                      <span>💧 ${(r.liquidityUsd / 1000).toFixed(0)}K liq</span>
                      <span>📊 ${(r.volume24h / 1000).toFixed(0)}K vol</span>
                      <span style={{ color: r.priceChange1h >= 0 ? theme.enter : theme.danger,
                        fontFamily: "var(--font-geist-mono), monospace" }}>
                        1h {r.priceChange1h >= 0 ? "+" : ""}{r.priceChange1h.toFixed(1)}%
                      </span>
                      {res.priceChange24h !== undefined && (
                        <span style={{ color: res.priceChange24h >= 0 ? theme.enter : theme.danger,
                          fontFamily: "var(--font-geist-mono), monospace" }}>
                          24h {res.priceChange24h >= 0 ? "+" : ""}{res.priceChange24h.toFixed(1)}%
                        </span>
                      )}
                    </div>

                    {/* AI reasoning */}
                    <div style={{ marginTop: 12, padding: "10px 12px", background: `${c}07`,
                      borderRadius: 10, border: `1px solid ${c}18` }}>
                      <div style={{ fontSize: 10, color: c, fontWeight: 700, textTransform: "uppercase",
                        letterSpacing: 0.5, marginBottom: 5 }}>🧠 AI Analysis</div>
                      ...
                      <div style={{ display: "flex", gap: 6 }}>
                        {res.dexscreenerUrl && (
                          <a href={res.dexscreenerUrl} target="_blank" rel="noreferrer"
                            style={{ padding: "4px 10px", borderRadius: 6, background: theme.panelAlt,
                        ...
                        {res.contractAddress && (
                          <button
                            ...
                            style={{ padding: "4px 14px", borderRadius: 6, border: "none", cursor: "pointer",
                              ...
                              background: r.action === "ENTER"
                                ? `linear-gradient(135deg, ${c}, ${c}CC)`
                                : `linear-gradient(135deg, ${theme.tierBasic}90, ${theme.tierPremium}90)`,
                              color: r.action === "ENTER" ? "#06070D" : theme.text,
                              boxShadow: r.action === "ENTER" ? `0 2px 12px ${c}30` : "none",
                              transition: "all 0.2s" }}
                            onMouseEnter={(e) => { e.currentTarget.style.transform = "translateY(-1px)"; }}
                            onMouseLeave={(e) => { e.currentTarget.style.transform = ""; }}
                          >
                            ↗ {r.chain === "hashkey" ? "HSKSwap" : "Swap"}
                          </button>
                        )}
                      </div>
                    </div>
